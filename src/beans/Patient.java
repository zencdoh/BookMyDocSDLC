...
public class Patient {
	...
	public boolean Login(int chk_id, String chk_password) throws Exception {
		...
		if (obj.getRowCount(query) == 0) {
			...
			rs.next();
			String corr_pass = rs.getString("p_password");
	...
	public int Register(String password1, int age1, String name1)
		...
		String insertquery = "INSERT INTO patient (id, p_password, p_name, p_age) VALUES ("
	...
	public int BookAppointment(String date1, String slot, int docid,
		...
		String insertquery = "Insert into appointment(id,doctor_id,patient_id,type,a_slot,a_date)values("
				+ ref
				+ ","
				+ docid
				+ ","
	...
	public boolean CheckAvailabity(String date1, String slot, int docid)
			throws Exception {
		obj.createConn();
		String checkquery = "Select * from appointment where doctor_id=" + docid
				+ " and a_date='" + date1 + "' and a_slot='" + slot + "'";
		if (obj.getRowCount(checkquery) == 0)
			return true;
		else
			return false;
